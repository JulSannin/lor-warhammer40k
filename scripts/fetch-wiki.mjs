...
const ALIASES = {
  ...
  // XVI легион в таблице назван двумя именами сразу. Отдельной статьи
  // о Лунных Волках нет, а Сыны Хоруса — перенаправление на Чёрный Легион
  'Лунные Волки → Сыны Хоруса': 'Чёрный Легион',
...
function collectTerms() {
  ...
  for (const section of content.sections) {
    for (const block of section.blocks) {
      ...
      /*
       * Названия легионов из таблицы примархов. Жирным они не выделены,
       * поэтому обычным сбором не ловятся, — но справка по ним нужна
       * не меньше, чем по самим примархам.
       */
      if (block.type === 'table' && block.head[0] === '№' && block.head[2] === 'Легион') {
        for (const row of block.rows) {
          const legion = (row[2] ?? '').replace(/[*_]/g, '').trim()
          // У II и XI легионов в этой клетке стоит «[стёрт]»
          if (legion && !legion.startsWith('[')) terms.add(legion)
        }
      }
    }
  }
